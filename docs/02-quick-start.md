...
Если нужны сведения о пропусках и ошибках, используйте `dispatchResults()`:

```php
use PhpSoftBox\Scheduler\ScheduleOutcome;

foreach ($scheduler->dispatchResults($time) as $result) {
...
Ошибка одной due-задачи не отменяет остальные. Она сохраняется в
`ScheduleResult::$exception`; CLI-команды также выводят её и продолжают цикл.
