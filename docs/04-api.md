...
- `dispatchResults(?DateTimeInterface $time = null): array` — выполнить due-задачи и вернуть результат каждого запуска или пропуска.
...
- `onOneServer(int $ttlSeconds = 120)` — разрешить запуск в данном due-window только одной реплике
...
Блокировки выключены по умолчанию и включаются только явным вызовом
`withoutOverlapping()` или `onOneServer()`.
...
Для `withoutOverlapping()` нужно передать `ScheduleLockStoreInterface` или
совместимый PSR-16 cache. У задачи должно быть стабильное имя; для invokable-класса
им может служить имя класса. Для closure лучше всегда задать имя явно:

```php
$scheduler->run(static function (): void {
    // ...
}, 'cleanup')->dailyAt('03:00')->withoutOverlapping();
```

`onOneServer()` решает другую задачу: lease не снимается после быстрого выполнения,
поэтому в пределах одной календарной минуты вторую реплику не пропустит даже уже
завершившаяся первая задача. Этот режим принимает только атомарный lock store.

## Результаты выполнения

`ScheduleResult` содержит:

- `taskName` и запланированное время `plannedAt`;
- фактические `startedAt`, `finishedAt` и вычисляемую `durationSeconds()`;
- `outcome`: `succeeded`, `queued`, `failed`, `skipped_overlapping` или
  `skipped_one_server`;
- возвращённое `value` либо исходное `exception`.

`dispatch()` оставлен как компактный совместимый API и возвращает только значения
успешных либо поставленных в очередь задач. Для мониторинга и production-кода
предпочтительнее `dispatchResults()`.
