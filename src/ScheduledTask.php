...
final class ScheduledTask implements ScheduleEntry
{
    ...
    private int $oneServerTtlSeconds = 120;
    ...
    public function named(string $name): self
    {
        $name = trim($name);
        if ($name === '') {
            throw new InvalidArgumentException('Task name must be non-empty.');
        }
    ...
    public function onOneServer(int $ttlSeconds = 120): self
    {
        if ($ttlSeconds < 1) {
            throw new InvalidArgumentException('One-server lease TTL must be >= 1.');
        }

        if ($this->name === null && $this->handlerClass === null) {
            throw new SchedulerException('onOneServer() requires a named task or a stable class handler.');
        }

        $this->oneServer           = true;
        $this->oneServerTtlSeconds = $ttlSeconds;
    ...
    public function oneServerTtlSeconds(): int
    {
        return $this->oneServerTtlSeconds;
    }

    public function lockId(): ?string
    {
        return $this->name ?? $this->handlerId;
    }

    public function onQueue(?string $queueName = null): self
    {
        if ($this->handlerClass === null) {
            throw new SchedulerException('Queued tasks must use invokable class handlers.');
        }
    ...
    {
        return ($this->handler)($this->resolveTime($time));
    }
